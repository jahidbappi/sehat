"""Shenzhen No.3 People's Hospital TB chest X-ray set (NLM "ChinaSet").

~662 frontal chest X-rays (336 abnormal with TB manifestations, 326 normal)
published by the U.S. National Library of Medicine. Images are named
``CHNCXR_XXXX_F.png`` with ``F=1`` indicating TB. Public domain; cite Jaeger
et al., 2014.
"""
...
__all__ = ["ShenzhenDataset"]

_FILENAME_RE = re.compile(r"^(CHNCXR_\d{4})_([01])\.png$", re.IGNORECASE)


class ShenzhenDataset(RawDataset):
    """NLM Shenzhen (ChinaSet) TB collection."""

    spec = DatasetSpec(
        name="shenzhen",
        site="shenzhen",
        disease="tb",
        urls=("https://openi.nlm.nih.gov/imgs/collections/ChinaSet_AllFiles.zip",),
